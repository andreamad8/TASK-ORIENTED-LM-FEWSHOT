...
def evaluate(data_src, data_ref, data_sys,
             print_as_table=True, print_table_header=False, sys_fname='',
             python=False):
...
if __name__ == '__main__':
    ...
    ref_file = ".data/NLG/reference_fixed.txt" 
    score_files = ["gpt2_1", "gpt2_10","gpt2_19", "gpt2-large_1", "gpt2-large_10","gpt2-large_19", "gpt2-xl_1", "gpt2-xl_10","gpt2-xl_19"]

    table = []
    for f_s in score_files:
        sys_file = f".data/NLG/NLG_{f_s}.txt"

        data_src, data_ref, data_sys = load_data(ref_file, sys_file, args.src_file)
        score = evaluate(data_src, data_ref, data_sys, args.table, args.header, sys_file, args.python)
        score["Model"] = f_s
        table.append(score)
    print(tabulate(table,headers="keys",tablefmt="plain",floatfmt=".4f"))
